...
const VideoDetail = ({ video }) => {
  ...
  useEffect(() => {
    if (video) {
      ...
      const storedViewData = localStorage.getItem(`viewData-${video.id}`);
      setViewData(storedViewData ? JSON.parse(storedViewData) : []);
    }
  }, [video]);
  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-4">
        <h3 className="text-xl font-bold mb-2">Views Growth Over Time</h3>
        <ViewsLineChart videoId={video.id} data={viewData} />
      </div>
    </div>
  );
};
